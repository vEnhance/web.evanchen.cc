\documentclass[11pt]{scrartcl}
\usepackage[sexy]{evan}

\begin{document}
\title{Lemmas in AoPS Geometry}
\subtitle{aka: how you can tell who's done too much olympiad geo}
\date{22 September 2026}
\maketitle

\begin{center}
  \includegraphics[width=0.95\textwidth]{slang.jpg}
\end{center}

\begin{abstract}
  This document is a compilation of a bunch of silly nicknames and terms
  that have become popular on the Internet among modern teenagers.
\end{abstract}

\section*{Important disclaimer (how to use this file)}
Before writing this handout, even I (Evan)
did not know several of the names here.
If you are a beginner or intermediate level geometry student,
\alert{I do not recommend memorizing the names or configurations in this file}.
The reason is:
\begin{itemize}
  \ii If it wasn't already in my textbook \cite{EGMO16} at least implicitly,
  then it doesn't come up often enough to be worth learning for beginners.
  \ii When it \emph{does} come up, intermediate students can often figure
  the relevant facts on-the-spot anyway. Thus, the only real value is
  documenting the ``slang'' name for a fact that they could derive on their own.
\end{itemize}
Instead, this handout is meant as an encyclopedic reference that you can refer to
when you hear some bizarre name on the Internet that does not appear in \cite{EGMO16}.

If you've completed \cite{EGMO16},
trying to prove the various properties listed here without checking the solutions
is also another reasonable use of this material.
That is, treating this as a source of exercises
that happen to also have silly names is above board.
This is especially true for \Cref{sec:named}.

\newpage

\tableofcontents

\newpage

\section{Basic facts that have acquired bizarre names}
\subsection{Reim's theorem}
Reim's theorem is a nickname given for a certain easy angle chasing statement.
To fully state it, we give the following somewhat silly definition.

\begin{definition}
  We say two lines $\ell_1$, $\ell_2$ are \emph{antiparallel}
  with respect to $m_1$ and $m_2$, if the four points $\ell_i \cap m_j$
  are the vertices of a cyclic quadrilateral.
\end{definition}

\begin{theorem}
  [Reim's theorem]
  Fix two lines $m_1$ and $m_2$.
  If $\ell_1$ is antiparallel to $\ell_2$ and $\ell_2$ is antiparallel to $\ell_3$,
  then lines $\ell_1$ and $\ell_3$ are either parallel or coincide.
\end{theorem}
\begin{proof}
  Because $\dang(m_1, \ell_1) = -\dang(m_2, \ell_2) = \dang(m_1, \ell_3)$.
\end{proof}

See \Cref{fig:reim}; I've highlighted the angles in green.
You'll notice this is literally a two-step angle chase,
which means that for practical purposes,
\alert{whenever you see the name ``Reim's theorem'',
it's just a shorthand for a certain two-step anglechase}.

So, if you ever come up to me with the excuse
``I didn't solve the problem because I didn't know Reim's theorem'',
I will be sassy in my reply.

\begin{figure}[ht]
  \centering
  \begin{asy}
    size(12cm);
    pair A = dir(110);
    pair B = dir(70);
    pair C = dir(30);
    pair D = dir(180);
    pair P = extension(A,D,B,C);
    real k = 5;
    pair U = (k+1)*A-k*P;
    pair V = (k+1)*B-k*P;

    draw(U--P--V, red+1.5);
    draw(A--B, blue+1.5);
    draw(C--D, blue+1.5);
    draw(U--V, blue+1.5);
    draw(unitcircle, grey);
    draw(circumcircle(U, V, C), grey);
    dot(P);
    dot(A);
    dot(B);
    dot(C);
    dot(D);
    dot(U);
    dot(V);

    path anglemark(pair A, pair B, pair C, real t=8) {
      pair M,N,P[],Q[];
      path mark;
      M=t*0.03*unit(A-B)+B;
      N=t*0.03*unit(C-B)+B;
      mark=arc(B,M,N);
      mark=(mark--B--cycle);
      return mark;
    }
    filldraw(anglemark(B,A,P,6), opacity(0.2)+yellow, deepgreen);
    filldraw(anglemark(P,C,D,6), opacity(0.2)+yellow, deepgreen);
    filldraw(anglemark(V,U,P,6), opacity(0.2)+yellow, deepgreen);
  \end{asy}
  \caption{The three angles of Reim's theorem.}
  \label{fig:reim}
\end{figure}

\subsection{The Incenter/Excenter Lemma / Fact 5 / Trillium theorem / chicken feet
theorem (EGMO Lemma 1.18)}

The following theorem has several slang names.
The most popular one, ``Fact 5'', originated from the filename
\texttt{Fact5.pdf} of my handout \cite{IEL16}
(despite the name itself not appearing in the handout at all).

\begin{lemma}[{\cite[Lemma 1.18]{EGMO16}}]
  Let $ABC$ be a triangle with incenter $I$, $A$-excenter $I_A$.
  The circumcenter of cyclic quadrilateral $IBI_AC$
  (that is, the midpoint of the diameter $\ol{II_A}$)
  coincides with the arc midpoint $L$ of minor arc $BC$.
\end{lemma}
\begin{figure}[ht]
  \centering
  \begin{asy}
    pair A = dir(110);
    pair B = dir(210);
    pair C = dir(330);
    pair I = incenter(A, B, C);
    draw(A--B--C--cycle);
    draw(unitcircle);
    pair L = dir(270);
    pair I_A = 2*L-I;
    filldraw(CP(L, I), opacity(0.2)+lightcyan, blue);

    draw(A--I_A, dotted);

    dot("$A$", A, dir(A));
    dot("$B$", B, dir(190));
    dot("$C$", C, dir(-10));
    dot("$I$", I, dir(60));
    dot("$L$", L, dir(225));
    dot("$I_A$", I_A, dir(I_A));

    /* Source generated by TSQ */
  \end{asy}
  \caption{\cite[Lemma 1.18]{EGMO16}, also known as ``Fact 5'',
  and many other regional nicknames.}
  \label{fig:fact5}
\end{figure}
\begin{proof}
  This is just angle chasing; see \Cref{fig:fact5}.
  Let $A = \angle BAC$, $B = \angle CBA$, $C = \angle ACB$,
  and note that $A$, $I$, $L$ are collinear (as $L$ is on the angle bisector).
  We are going to show that $LB = LI$, the other cases being similar.

  First, notice that
  \[ \angle LBI = \angle LBC + \angle CBI
    = \angle LAC + \angle CBI = \angle IAC + \angle CBI = \half A + \half B. \]
  However,
  \[ \angle BIL = \angle BAI + \angle ABI = \half A + \half B. \]
  Hence, $\triangle BIL$ is isosceles.
  So $LB = LI$.
  The rest of the proof proceeds along these lines.
\end{proof}
You can remember this lemma as saying that the
\alert{circumcenter of $BIC$ is the arc midpoint}.

\subsection{Iran incenter lemma (EGMO Lemma 1.45)}
The following lemma in EGMO earned the nickname ``Iran lemma''
or ``Iran incenter lemma'' because of its prominence in \cite[Problem 11.19]{EGMO16}
(which was from an Iran TST, and from the last chapter of EGMO).
This lemma also featured as USAJMO 2014 problem 6.

\begin{figure}[ht]
  \centering
  \begin{asy}
    pair A = dir(130);
    pair B = dir(210);
    pair C = dir(330);
    pair I = incenter(A, B, C);
    pair D = foot(I, B, C);
    pair E = foot(I, C, A);
    pair F = foot(I, A, B);
    pair M = midpoint(B--C);
    pair N = midpoint(C--A);
    pair K = extension(M, N, E, F);

    filldraw(A--B--C--cycle, opacity(0.1)+lightcyan, blue);
    draw(incircle(A, B, C), blue);
    draw(M--K, red);
    draw(F--K, red);
    draw(B--K--C, red);

    dot("$A$", A, dir(A));
    dot("$B$", B, dir(B));
    dot("$C$", C, dir(C));
    dot("$I$", I, dir(315));
    dot("$D$", D, dir(D));
    dot("$E$", E, dir(70));
    dot("$F$", F, dir(F));
    dot("$M$", M, dir(M));
    dot("$N$", N, dir(N));
    dot("$K$", K, dir(K));

    /* -----------------------------------------------------------------+
    |                 TSQX: by CJ Quines and Evan Chen                  |
    | https://github.com/vEnhance/dotfiles/blob/main/py-scripts/tsqx.py |
    +-------------------------------------------------------------------+
    A = dir 130
    B = dir 210
    C = dir 330
    I 315 = incenter A B C
    D = foot I B C
    E 70 = foot I C A
    F = foot I A B
    M = midpoint B--C
    N = midpoint C--A
    K = extension M N E F
    A--B--C--cycle / 0.1 lightcyan / blue
    incircle A B C / blue
    M--K / red
    F--K / red
    B--K--C / red
    */
  \end{asy}
  \caption{The Iran incenter lemma.}
  \label{fig:iran}
\end{figure}

\begin{lemma}[{\cite[Lemma 1.45]{EGMO16}}]
  The incircle of $\triangle ABC$ is tangent to $\ol{BC}$, $\ol{CA}$, $\ol{AB}$
  at $D$, $E$, $F$, respectively.
  Let $M$ and $N$ be the midpoints of $\ol{BC}$ and $\ol{AC}$, respectively.
  Ray $BI$ meets line $EF$ at $K$.
  Then $\ol{BK} \perp \ol{CK}$ and $K$ lies on $MN$.
\end{lemma}
\begin{proof}
  See \Cref{fig:iran}.
  Left as exercise because it's a problem in EGMO.
  Hint: focus on the circle with diameter $\ol{IC}$,
  which passes through $D$, $E$, and $K$.
\end{proof}

A good way to remember this lemma is that:
\begin{quote}
\alert{The $A$-touch chord, $B$-bisector, and $C$-midline concur}.
\end{quote}

\subsection{Shooting lemma / Death Star lemma (EGMO Lemma 4.33)}
This lemma concerns the picture in which you have two internally
tangent circles and a segment of the large circle tangent to the smaller one.
In Brazil, it is called the \alert{Death Star lemma} because
the two circles look like the Death Star spaceship from Star Wars.
Here it is:
\begin{lemma}[{\cite[Lemma 4.33]{EGMO16}}]
  Let $\ol{AB}$ be a chord of a circle $\Omega$.
  Let $\omega$ be a circle tangent to chord $\ol{AB}$ at $K$
  and internally tangent to $\omega$ at $T$.
  Then ray $TK$ passes through the midpoint $M$ of the arc $\widehat{AB}$ not containing $T$.
  Moreover, \[ MA^2 = MB^2 = MK \cdot MT. \]
\end{lemma}

\begin{figure}[ht]
  \centering
  \begin{asy}
    size(6cm);
    pair A = dir(170);
    pair B = dir(10);
    pair M = dir(270);
    pair T = dir(55);
    pair K = extension(M, T, A, B);
    filldraw(unitcircle, opacity(0.1)+cyan, blue);
    pair V = K+dir(90);
    pair P = extension(origin, T, K, V);
    filldraw(CP(P, K), opacity(0.1)+yellow, deepgreen);
    draw(A--B, blue);
    draw(M--T, red);
    draw(B--M--A, grey);

    dot("$A$", A, dir(A));
    dot("$B$", B, dir(B));
    dot("$M$", M, dir(M));
    dot("$T$", T, dir(T));
    dot("$K$", K, dir(315));
    dot("$P$", P, dir(P));

    /* -----------------------------------------------------------------+
    |                 TSQX: by CJ Quines and Evan Chen                  |
    | https://github.com/vEnhance/dotfiles/blob/main/py-scripts/tsqx.py |
    +-------------------------------------------------------------------+
    A = dir 170
    B = dir 10
    M = dir 270
    T = dir 55
    K 315 = extension M T A B
    unitcircle / 0.1 cyan / blue
    V := K+dir(90)
    P = extension origin T K V
    CP P K / 0.1 yellow / deepgreen
    A--B / blue
    M--T / red
    B--M--A / grey
    */
  \end{asy}
  \caption{The shooting lemma / death star lemma.}
  \label{fig:shooting}
\end{figure}

\begin{proof}
  See \Cref{fig:shooting}.
  The fact that $TK$ passes through the arc midpoint
  follows by taking the homothety at $T$ mapping $\omega$ to $\Omega$:
  it should map the ``south pole'' of $\omega$
  to that of $\Omega$, ergo it maps $M$ to $K$.

  Meanwhile, $MK \cdot MT = MA^2$ follows from
  $\triangle MKA \sim \triangle MAT$.
\end{proof}
In the United States, the name \alert{shooting lemma} refers to various
parts of this lemma (I never figured out which).

\subsection{Salmon theorem}
The name ``Salmon theorem'' refers to a certain easy spiral similarity.
It seems to have first appeared in the official solutions for USAMO 2013
provided by the MAA. Here Salmon is a surname, not a kind of fish.

\begin{figure}[ht]
  \centering
  \begin{asy}
    pair A = dir(125);
    pair B = dir(210);
    pair C = dir(330);
    pair P = 0.3*C+0.7*B;
    filldraw(A--B--C--cycle, opacity(0.1)+lightcyan, blue);

    pair O_1 = circumcenter(A, B, P);
    pair O_2 = circumcenter(A, C, P);

    real r1 = abs(P-O_1);
    real r2 = abs(P-O_2);
    pair Se = (r1*O_2-r2*O_1)/(r1-r2);
    path w1 = CP(O_1,P);
    path w2 = CP(O_2,P);
    filldraw(w1, opacity(0.1)+yellow, deepgreen);
    filldraw(w2, opacity(0.1)+yellow, deepgreen);
    filldraw(A--O_1--O_2--cycle, opacity(0.1)+lightred, red);
    draw(A--P, grey);

    dot("$A$", A, dir(A));
    dot("$B$", B, dir(B));
    dot("$C$", C, dir(C));
    dot("$P$", P, dir(-90));
    dot("$O_1$", O_1, dir(-90));
    dot("$O_2$", O_2, dir(-90));
  \end{asy}
  \caption{Salmon's theorem.}
  \label{fig:salmon}
\end{figure}

\begin{theorem}
  [Salmon theorem]
  Let $ABC$ be a triangle and $P$ a point on line $BC$.
  Then we have the spiral similarity
  \[ \triangle AO_1B \overset{+}{\sim} \triangle AO_2C, \qquad
    \triangle ABC \overset{+}{\sim} \triangle AO_1O_2. \]
\end{theorem}
\begin{proof}
  See \Cref{fig:salmon}.
  Assume without loss of generality that $\angle APB \le 90\dg$.
  Then \[ \angle AO_1B = 2 \angle APB \]
  but \[ \angle AO_2C = 2 \left( 180 - \angle APC \right) = 2 \angle ABP. \]
  Hence $\angle AO_1B = \angle AO_2C$.
  Moreover, both triangles are isosceles, establishing the first similarity.

  The second part follows from spiral similarities coming in pairs.
\end{proof}

Confusingly, there are other results named ``Salmon theorem'',
see, e.g.\ Wolfram Mathworld's entry \cite{ST}.

\subsection{The first isogonality lemma}
The following lemma appears in \emph{Geometry Revisited} as
\cite[Section 1.9, Exercise 3]{GR67}.
It comes up once in a blue moon, and has earned the name ``isogonality lemma''.
\begin{lemma}
  [The first isogonality lemma]
  Let $ABC$ be a triangle and let $P$ be a point inside
  it satisfying $\angle ABP = \angle PCA$.
  Let $Q$ be the reflection of $P$ across the midpoint of $\overline{BC}$.
  Then $\angle BAP = \angle CAQ$.
\end{lemma}
\begin{proof}
  Most straightforward would be to use barycentric coordinates,
  but you can see a synthetic proof in British MO 2013 on AOPS \cite{BMO13}.
\end{proof}

The condition $\angle ABP = \angle ACP$ is actually
usually more naturally interpreted as saying the points
$B$, $C$, $\ol{BP} \cap \ol{AC}$ and $\ol{CP} \cap \ol{AB}$ are cyclic.
Similarly, the parallelogram $BPCQ$ is noteworthy.

\begin{figure}[ht]
  \centering
  \begin{asy}
  size(7cm);
  pair Y = dir(70);
  pair Z = dir(130);

  pair B = dir(170);
  pair C = dir(10);

  pair A = extension(B, Z, C, Y);
  pair P = extension(B, Y, C, Z);
  pair Q = B+C-P;
  filldraw(A--B--C--cycle, opacity(0.1)+lightgreen, deepgreen);
  filldraw(B--P--C--Q--cycle, opacity(0.1)+yellow, heavygreen);
  draw(P--Q, heavygreen);
  draw(Z--P--Y, blue+dashed);

  filldraw(circumcircle(B, Z, Y), opacity(0.1)+lightcyan, deepcyan+dashed);

  draw(P--A--Q, blue);

  dot(Y);
  dot(Z);
  dot("$B$", B, dir(B));
  dot("$C$", C, dir(C));
  dot("$A$", A, dir(A));
  dot("$P$", P, dir(250));
  dot("$Q$", Q, dir(Q));

  clip(box((-1.2,-0.4),(1.2,2)));

  /* TSQ Source:

  Y .= dir 70
  Z .= dir 130

  B = dir 170
  C = dir 10

  A = extension B Z C Y
  P = extension B Y C Z R250
  Q = B+C-P
  A--B--C--cycle 0.1 lightgreen / deepgreen
  B--P--C--Q--cycle 0.1 yellow / heavygreen
  P--Q heavygreen
  Z--P--Y blue dashed

  circumcircle B Z Y 0.1 lightcyan / deepcyan dashed

  P--A--Q blue

  */
  \end{asy}
  \label{fig:isog}
  \caption{The first isogonality lemma.}
\end{figure}

\begin{remark}
  The \emph{second} isogonality lemma refers to the following statement:
  Suppose that $\ol{AP}$ and $\ol{AQ}$
  are isogonal with respect to $\angle A$.
  Let $X = \ol{PB} \cap \ol{QC}$ and $Y = \ol{PC} \cap \ol{QB}$.
  Then $\ol{AX}$ and $\ol{AY}$ are isogonal with respect to $\angle A$.
\end{remark}

\newpage
\section{Length-based lemmas}
\subsection{Ratio lemma}
In the United States, the following fact is sometimes called the ratio lemma.
Like Reim's theorem, this name is ``unnecessary'' in that it is only
capturing a single natural step.
The lemma is most prominently featured in \emph{Ratio Lemma} by \texttt{mira74} \cite{RL20}.

\begin{figure}[ht]
  \centering
  \begin{asy}
    size(6cm);
    pair A = dir(125);
    pair B = dir(210);
    pair C = dir(330);
    pair D = 0.3*C+0.7*B;
    filldraw(A--B--C--cycle, opacity(0.1)+lightcyan, blue);
    draw(A--D, grey);
    dot("$A$", A, dir(A));
    dot("$B$", B, dir(B));
    dot("$C$", C, dir(C));
    dot("$D$", D, dir(-90));
    markangle(1,19.0,B,A,D,deepgreen);
    markangle(2,12.0,D,A,C,deepgreen);
  \end{asy}
  \caption{The simple picture for the Ratio Lemma.}
  \label{fig:ratio}
\end{figure}

\begin{lemma}
  [Ratio lemma]
  Let $ABC$ be a triangle and $D$ a point in the interior of segment $BC$. Then
  \[ \frac{BD}{DC} = \frac{AB}{AC} \frac{\sin \angle BAD}{\sin \angle CAD}. \]
\end{lemma}
\begin{proof}
  See \Cref{fig:ratio}.
  By the law of sines on $\triangle ABD$ and $\triangle ACD$, we have
  \[
    \frac{BD}{\sin \angle BAD} = \frac{AB}{\sin \angle ADB}, \qquad
    \frac{CD}{\sin \angle CAD} = \frac{AC}{\sin \angle ADC}.
  \]
  Since $\angle ADB + \angle ADC = 180\dg$, canceling those sines finishes.
\end{proof}

\subsection{Prism lemma}
The following simple lemma from projective geometry is useful for
transferring cross ratios.
The name is because the picture looks like a pyramid (see \Cref{fig:prism}).
I'm told that the origin story came from me mentioning during the test review
of TSTST 2019/9 that ``this motif comes up often, so we should have a name for it'',
and one student suggested ``prism lemma'', which stuck.

\begin{figure}[ht]
  \centering
  \begin{asy}
    pair D = (0,4);
    pair A1 = (-1,0);
    pair A2 = (2,0);
    pair C1 = 0.7*D+0.3*A1;
    pair B1 = 0.5385*D+0.4615*A1;
    pair T = (6,0);
    pair B2 = extension(T, B1, D, A2);
    pair C2 = extension(T, C1, D, A2);
    draw(A1--D--A2, blue);
    draw(T--A1, lightred);
    draw(T--B1, lightred);
    draw(T--C1, lightred);
    dot(A1);
    dot(A2);
    dot(B1);
    dot(B2);
    dot(C1);
    dot(C2);
    dot(D);
    dot(T);
  \end{asy}
  \caption{The prism lemma.}
  \label{fig:prism}
\end{figure}

\begin{lemma}
  [Prism lemma]
  Two lines $\ell_1$ and $\ell_2$ meet at a point $D$.
  Let $A_1$, $B_1$, $C_1$ be distinct points on $\ell_1$
  and $A_2$, $B_2$, $C_2$ be distinct points on $\ell_2$, different from $D$.
  Then $A_1 A_2$, $B_1 B_2$, $C_1 C_2$ are concurrent
  if and only if $(A_1B_1; C_1D) = (A_2B_2; C_2D)$.
\end{lemma}
\begin{proof}
  This is a restatement of the fact that
  projection through a point preserves cross ratios.
\end{proof}

\subsection{EFFT (EGMO Theorem 7.16 and 7.25)}
In barycentric coordinates, there are two theorems in EGMO
that let one decide whether two displacement vectors are perpendicular.

Teenager Evan, in 2012, thought it was a good idea to call these
``EFFT'' and ``strong EFFT'' respectively,
in the handout he posted on Art of Problem Solving \cite{BCOG12}.
Later efforts to get rid of those names from the slang repository
have been largely unsuccessful (even though the names do not appear in \cite{EGMO16}),
and adult Evan has given up.

\begin{theorem}[{\cite[Lemma 7.16]{EGMO16}}]
  Let $\overrightarrow{MN} = (x_1, y_1, z_1)$
  and $\overrightarrow{PQ} = (x_2, y_2, z_2)$ be displacement vectors.
  Then $\ol{MN} \perp \ol{PQ}$ if and only if
  \[  0 = a^2(z_1y_2 + y_1z_2) + b^2(x_1z_2 + z_1x_2) + c^2(y_1x_2 + x_1y_2). \]
\end{theorem}

\begin{theorem}[{\cite[Lemma 7.25]{EGMO16}}]
  Suppose $M$, $N$, $P$, and $Q$ are points with
  \begin{align*}
    \overrightarrow{MN} &= x_1 \overrightarrow{AO} + y_1 \overrightarrow{BO} + z_1 \overrightarrow{CO} \\
    \overrightarrow{PQ} &= x_2 \overrightarrow{AO} + y_2 \overrightarrow{BO} + z_2 \overrightarrow{CO}
  \end{align*}
  such that either $x_1+y_1+z_1=0$ or $x_2+y_2+z_2=0$.
  In that case, lines $MN$ and $PQ$ are perpendicular if and only if
  \[ 0 = a^2(z_1y_2 + y_1z_2) + b^2(x_1z_2 + z_1x_2) + c^2(y_1x_2 + x_1y_2). \]
\end{theorem}

\newpage

\section{Special named points with respect to a triangle vertex}
\label{sec:named}

We define some additional slang names of points.
These points have many, many properties that we defer to external references.
Here, I try and name the 1--3 ``most important'' properties (beyond the definition).
The way I try to pick ``most important'' is (i) the property is easy to remember;
(ii) additional facts in literature are mostly within
one or two logical steps from one of mine.

Hints for proofs of the theorems are in \Cref{sec:hints}.
They use theory from all of \cite{EGMO16}, particularly projective geometry.

\subsection{Humpty and Dumpty points}
As shown in \Cref{fig:HM}, let $ABC$ be a triangle with orthic triangle $DEF$,
and orthocenter $H$. Let $M$ be the midpoint of $\ol{BC}$.
\begin{definition}
  The foot of the altitude from $H$ to $\ol{AM}$,
  denoted $H_A$ in \Cref{fig:HM},
  is called the \alert{$A$-Humpty point}.
  (An older name is the $A$-HM point.)
\end{definition}
\begin{theorem}
  [Most important properties of the $A$-Humpty point]
  \label{thm:humpty}
  In the notation of \Cref{fig:HM}:
  \begin{enumerate}
    \ii The points $B$, $H$, $C$, $H_A$ are concyclic.
    \ii Line $BC$ is tangent to both $(AH_AB)$ and $(AH_AC)$.
    Equivalently,
    \begin{equation}
      \dang H_AAC = \dang H_ACB, \quad\text{and}\quad \dang H_AAB = \dang H_ABC.
      \label{eq:humpty_angle}
    \end{equation}
  \end{enumerate}
\end{theorem}

\begin{figure}[ht]
  \centering
  \begin{asy}
    size(10cm);
    pair A = dir(110);
    pair B = dir(210);
    pair C = dir(330);
    pair M = midpoint(B--C);
    pair H = orthocenter(A, B, C);
    pair G = foot(A, M, H);
    pair H_A = foot(H, A, M);
    pair D = foot(A, B, C);
    pair E = foot(B, C, A);
    pair F = foot(C, A, B);

    filldraw(A--B--C--cycle, opacity(0.1)+lightred, red);
    filldraw(unitcircle, opacity(0.1)+yellow, red);
    draw(A--D, red);
    draw(B--E, red);
    draw(C--F, red);

    pair X = extension(E, F, B, C);
    draw(A--M, red);
    draw(A--X--H_A, orange+dashed);
    draw(E--X--B, red);
    draw(G--M, orange+dashed);

    filldraw(circumcircle(A, E, F), opacity(0.1)+yellow, orange);
    draw(B..H..H_A..C, gray);

    dot("$A$", A, dir(A));
    dot("$B$", B, dir(B));
    dot("$C$", C, dir(C));
    dot("$M$", M, dir(M));
    dot("$H$", H, dir(H));
    dot("$H_A$", H_A, dir(335));
    dot("$D$", D, dir(D));
    dot("$E$", E, dir(20));
    dot("$F$", F, dir(F));
    dot("$X$", X, dir(X));
    dot("$G$", G, dir(G));

    /* TSQ Source:

    A = dir 110
    B = dir 210
    C = dir 330
    M = midpoint B--C
    H = orthocenter A B C
    G = foot A M H
    H_A = foot H A M R335
    D = foot A B C
    E = foot B C A R20
    F = foot C A B

    A--B--C--cycle 0.1 lightred / red
    unitcircle 0.1 yellow / red
    A--D red
    B--E red
    C--F red

    X = extension E F B C
    A--M red
    A--X--H_A orange dashed
    E--X--B red
    G--M orange dashed

    circumcircle A E F 0.1 yellow / orange

    */
  \end{asy}
  \caption{The Humpty point (denoted $H_A$) and the $A$-queue point (denoted $G$).}
  \label{fig:HM}
\end{figure}

The isogonal conjugate of the $A$-Humpty point is appropriately named:
\begin{definition}
  The \alert{$A$-Dumpty point} $Q_A$ is the isogonal conjugate of $H_A$.
\end{definition}
Using the definition of isogonal conjugate to rewrite \eqref{eq:humpty_angle} gives
\begin{equation}
  \dang Q_A B A = \dang Q_A A C, \quad \text{and} \quad \dang Q_A C A = \dang Q_A A B.
  \label{eq:dumpty_angle}
\end{equation}
However, the real important property of the $A$-Dumpty point (not pictured) is:
\begin{theorem}
  [Most important property of the $A$-Dumpty point]
  \label{thm:dumpty}
  The $A$-Dumpty point $Q_A$ coincides with the midpoint of the $A$-symmedian chord.
\end{theorem}

References for these points include:
\begin{itemize}
  \ii This math.SE answer: \cite{SE20}.
  \ii \emph{On Two Special Points in a Triangle}, by Kapil Pause \cite{TSP18},
  which seems to be the origin of the names Humpty and Dumpty.
  \ii \emph{A Special Point on the Median} by Gunmay Handa and Anant Mudgal
  \cite{SPM17}.
  \ii \emph{Midpoint of Symmedian Chord}, by Srijon Sarkar \cite{MSC21}.
\end{itemize}

\subsection{Queue and ex points}
In the Humpty configuration, the point $G$ in \Cref{fig:HM}
also plays an important role:
\begin{definition}
  The \alert{$A$-queue point} is the second intersection of $(AEF)$ and $(ABC)$,
  denoted $G$ in \Cref{fig:HM}.
  In other words, it's the Miquel point of cyclic quadrilateral $BFEC$.
\end{definition}
\begin{theorem}
  [Most important properties of the $A$-queue point]
  \label{thm:queue}
  In the notation of \Cref{fig:HM}:
  \begin{enumerate}
    \ii Lines $AG$, $EF$, $HH_A$, $BC$ meet at the point $X$ satisfying $(XD;BC) = -1$.
    \ii Point $G$ lies on line $HM$, and $\ol{MHG} \perp \ol{AGX}$.
  \end{enumerate}
\end{theorem}
The point $X$ is called the \alert{$A$-ex point} sometimes.
The names seem to have come from a blog post by \texttt{math\_pi\_rate} \cite{EQ18},
which documents several additional properties.

\subsection{Sharky-Devil point}
As shown in \Cref{fig:sharkydevil},
let $ABC$ be a triangle with intouch triangle $DEF$.
\begin{definition}
  The \alert{$A$-Sharkydevil point}, denoted $K$ in \Cref{fig:sharkydevil},
  is the second intersection of $(AEF)$ and $(ABC)$.
  In other words, it's the Miquel point of $BFEC$.
\end{definition}
The name seems coined by \texttt{aops29} in a blog post \cite{SDP19}.

\begin{figure}[ht]
  \centering
  \begin{asy}
    size(10cm);
    pair A = dir(130);
    pair B = dir(210);
    pair C = dir(330);
    filldraw(unitcircle, opacity(0.1)+palecyan, lightblue);
    draw(A--B--C--cycle, lightblue);
    pair I = incenter(A, B, C);
    pair D = foot(I, B, C);
    draw(incircle(A, B, C), lightblue);
    pair M = dir(270);
    pair K = extension(-A, I, M, D);
    pair E = foot(I, A, C);
    pair F = foot(I, A, B);
    draw(circumcircle(A, E, F), heavygreen);
    pair S = -A;
    pair P = foot(D, E, F);
    draw(S--K, lightblue);
    draw(K--M, heavygreen);
    draw(B--K--C, lightblue);
    draw(D--E--F--cycle, heavycyan);
    draw(D--P, heavycyan);

    pair L = dir(90);
    pair Y = foot(M, L, D);
    draw(A--Y, purple);
    draw(Y--L, purple);

    pair T = extension(A, K, B, C);
    draw(T--I, dotted);
    draw(A--T--B, dotted);

    pair R = extension(L, I, B, C);
    draw(L--R, gray);

    dot("$A$", A, dir(A));
    dot("$B$", B, dir(B));
    dot("$C$", C, dir(C));
    dot("$I$", I, dir(270));
    dot("$D$", D, dir(330));
    dot("$M$", M, dir(M));
    dot("$K$", K, dir(K));
    dot("$E$", E, dir(45));
    dot("$F$", F, dir(200));
    dot("$P$", P, dir(P));
    dot("$L$", L, dir(L));
    dot("$Y$", Y, dir(Y));
    dot("$T$", T, dir(T));
    dot("$R$", R, dir(R));

    /* --------------------------------+
    | TSQX: by Evan Chen and CJ Quines |
    | https://github.com/vEnhance/tsqx |
    +----------------------------------+
    A = dir 130
    B = dir 210
    C = dir 330
    unitcircle / 0.1 palecyan / lightblue
    A--B--C--cycle / lightblue
    I 270 = incenter A B C
    D 330 = foot I B C
    incircle A B C / lightblue
    M = dir 270
    K = extension -A I M D
    E 45 = foot I A C
    F 200 = foot I A B
    circumcircle A E F / heavygreen
    S := -A
    P = foot D E F
    S--K / lightblue
    K--M / heavygreen
    B--K--C / lightblue
    D--E--F--cycle / heavycyan
    D--P / heavycyan

    L = dir 90
    Y = foot M L D
    A--Y / purple
    Y--L / purple

    T = extension A K B C
    T--I / dotted
    A--T--B / dotted

    R = extension L I B C
    L--R / gray
    */
  \end{asy}
  \caption{The $A$-Sharkydevil point (denoted $K$) and its harmonic conjugate, the Evan is Old point (denoted $Y$).}
  \label{fig:sharkydevil}
\end{figure}

\begin{theorem}
  [Most important properties of the Sharkydevil point]
  \label{thm:sharkydevil}
  In the notation of \Cref{fig:sharkydevil}:
  \begin{enumerate}
    \ii Line $\ol{KD}$ bisects $\angle BKC$
    and hence passes through the midpoint $M$ of $\arc{BC}$.
    \ii Let $P$ denote the foot from $D$ to $\ol{EF}$.
    Then $K$ is the inverse of $P$ with respect to the incircle.
    \ii We have $(AK,AP;AE,AF) = -1$.
    \ii Lines $AK$ and $BC$ meet at a point $T$ such that $\angle AIT = 90\dg$.
  \end{enumerate}
\end{theorem}
Note that the first property of the preceding theorem
implies that $KDM$ passes through $X_{56}$,
the center of the positive homothety between the incircle and circumcircle.
(This is said homothety, centered at $X_{56}$, maps the south poles $D$ and $M$.)

\subsection{Evan Is Old point}
Retain \Cref{fig:sharkydevil}.
The following passage appears in \emph{’Muricaaaaaaa} by \texttt{i3435} \cite{M21}:
\begin{quote}
  \slshape
  The next point I'll bring up in this section is $E_O$,
  the intersection of $\ol{AP}$ and $(ABC)$.
  The reason this is named as such is because it showed up in TSTST 2020/2,
  and Evan Chen said he hadn't seen it before, which means that it isn't well known\dots
  Thus the point is named $E_O$ for Evan is Old.
\end{quote}
Unfortunately, I finally found this passage only in September 2026,
more than five years later, and too late to stop the spread of the name.
Nonetheless, n protest of the notation $E_O$, I instead use $Y$ for ``Young''.
\begin{definition}
  The \alert{$A$-Evan Is Old point}, denoted $Y$, is the second intersection
  of line $AP$ with the circumcircle of $ABC$.
\end{definition}

\begin{theorem}
  [Most important properties of the Evan is Old point]
  \label{thm:evanisold}
  In the notation of \Cref{fig:sharkydevil}:
  \begin{enumerate}
    \ii We have the harmonic quadrilateral $(KY;BC) = -1$.
    \ii Line $YD$ passes through the midpoint $L$ of arc $BAC$ (the antipode of $M$).
    \ii Lines $APY$ and $IL$ meet at the point $R$ on $BC$ such that $(TR;BC) = -1$.
  \end{enumerate}
\end{theorem}

\section{’Muricaaaaaaa, by \texttt{i3435}}
I tried to keep this handout brief and to-the-point,
since I intended it as a lookup of common idiosyncratic names.

If you want a more complete reference,
read \emph{’Muricaaaaaaa} by \texttt{i3435} \cite{M21}.
It is \emph{much} more comprehensive, including proofs of the facts here,
several properties of the named points not recorded here, and many more configurations.
It also features a long set of problems and solutions.

\begin{thebibliography}{EGMO16}
\bibitem[BCOG12]{BCOG12}
  Evan Chen and Max Schindler.
  \emph{Barycentric Coordinates in Olympiad Geometry}, 2012.
  \url{https://web.evanchen.cc/handouts/bary/bary-full.pdf}.

\bibitem[BMO13]{BMO13}
  Art of Problem Solving thread for \textit{British MO 2013}, 2013.
  \url{https://aops.com/community/c6h518987}.

\bibitem[EGMO16]{EGMO16}
  Evan Chen.
  \emph{Euclidean Geometry in Mathematical Olympiads}, 2016.
  MAA Problem Book Series.
  \url{https://web.evanchen.cc/geombook.html}

\bibitem[EQ18]{EQ18}
  \texttt{math\_pi\_rate}.
  \textit{The Ex-points and the Queue-points}, 2018.
  \url{https://aops.com/community/c701535h1754217}.

\bibitem[GR67]{GR67}
  H.\ S.\ M.\ Coxeter and S.\ L.\ Greitzer.
  \emph{Geometry Revisited}, 1967.

\bibitem[IEL16]{IEL16}
  Evan Chen. \textit{The Incenter/Excenter Lemma}, 2016.
  \url{https://web.evanchen.cc/handouts/Fact5/Fact5.pdf}.

\bibitem[M21]{M21}
  \texttt{i3435}. \textit{’Muricaaaaaaa}, 2021.
  \url{https://aops.com/community/p20459825}.

\bibitem[MSC21]{MSC21}
  Srijon Sarkar. \textit{Midpoint of Symmedian Chord}.
  Mathematical Reflections, issue 4 of 2021.
  \url{https://www.awesomemath.org/wp-pdf-files/math-reflections/mr-2021-04/mr_4_2021_midpoint_of_symmedian_chord.pdf}.

\bibitem[RL20]{RL20}
  \texttt{mira74}. \textit{Ratio Lemma}, 2020.
  \url{https://aops.com/community/p19166714}.

\bibitem[SDP19]{SDP19}
  \texttt{aops29}. \textit{Properties Of The Sharky-Devil Point}, 2019.
  \url{https://aops.com/community/c946900h1911664}.

\bibitem[SE20]{SE20}
  Math StackExchange answer 3894697, 2020.
  \url{https://math.stackexchange.com/a/3894697/229197}.

\bibitem[SPM17]{SPM17}
  Gunmay Handa and Anant Mudgal.
  \textit{A Special Point on the Median}, 2017.
  (The link for this file seems to be offline now.)

\bibitem[ST]{ST}
  Eric W.\ Weisstein. \textit{Salmon's Theorem}.
  MathWorld---A Wolfram Web Resource.
  \url{https://mathworld.wolfram.com/SalmonsTheorem.html}.

\bibitem[TSP18]{TSP18}
  Kapil Pause. \textit{On Two Special Points in a Triangle}, 2018.
  \url{https://pregatirematematicaolimpiadejuniori.files.wordpress.com/2018/05/two_special_points__1_.pdf}.
\end{thebibliography}

\newpage

\appendix
\section{One-sentence hints for proofs in \Cref{sec:named}}
\label{sec:hints}

I think it is better for you to try these yourself as exercises,
but here are some one-sentence hints if you're feeling stuck on a part.

\subsection{Humpty point (\Cref{thm:humpty}) and Dumpty point (\Cref{thm:dumpty})}
For Humpty (\Cref{thm:humpty}):
\begin{enumerate}
  \ii Use radical axis on $(EFHH_A)$, $(HH_AM)$, $(EFBC)$, $(EFDM)$ to define $X$
  and finish with power of a point from $X$.
  \ii After the previous item, this is direct angle chasing.
\end{enumerate}
For Dumpty (\Cref{thm:dumpty}), let $L$ be the point on $(ABC)$ so that $(AL;BC) = -1$.
\begin{quote}
  Notice that \eqref{eq:dumpty_angle} gives $\dang Q_A AC = \dang LAC = \dang LBC$,
  so $\ol{BQ_A}$ is the $B$-median of $\triangle BAL$.
\end{quote}

\subsection{Queue point (\Cref{thm:queue})}
\begin{enumerate}
  \ii Use radical axis on $(AEF)$, $(BFEC)$, $(ABC)$, $(BHH_AC)$.
  \ii By Brokard theorem on $BEFC$.
\end{enumerate}

\subsection{Sharkydevil point (\Cref{thm:sharkydevil})}
\begin{enumerate}
  \ii By spiral similarity, note that \[ \frac{KB}{KC} = \frac{DF}{CE} = \frac{BD}{DC}. \]
  \ii Inversion around the incircle maps $(ABC)$ to the nine-point circle of $DEF$
  and $(AEF)$ to the line $EF$, so it maps $K$ to $P$.
  \ii Noting $AFIE$ is a kite, project $-1 = (AI;EF) \overset{K}{=} (AK \cap EF, P; EF)$.
  \ii Use radical axis on $(AKFIE)$, $(BIC)$, $(AKBC)$
  since the former two circles are tangent at $I$.
\end{enumerate}

\subsection{Evan Is Old point (\Cref{thm:evanisold})}
\begin{enumerate}
  \ii Project $(AK,AP;AE,AF) = -1$ through $A$ onto the circumcircle.
  \ii Noting $LBMC$ is a kite, project $(ML;CB) = -1$ and $(KY;BC) = -1$ through $D$.
  \ii First, $R \in \ol{APY}$ follows from the first part.
  Second, $R \in \ol{LI}$ follows since $\ol{LI}$ is the $I$-symmedian of $\triangle IBC$
  and $\ol{TI}$ is tangent to $(IBC)$ at $I$.

  (In \cite{M21}, the proof of this item additionally incorporates mixtilinear theory.
  Our proof here does not do this to keep things simpler.)
\end{enumerate}

\end{document}
