...
\begin{proof}
  ...
  Set $f(x) = \frac{1}{16x} - \frac{1}{(x+3)^2}$.
  We can check that $f$ over $(0,3)$ is convex so Jensen completes the problem.
\end{proof}
\begin{example}
...
\end{example}
\begin{proof}
  Again assume WLOG that $a \ge b \ge c$ and notice that $(a+b-c,c+a-b,b+c-a) \succ (a,b,c)$.
  Apply Karamata on $f(x) = \sqrt x$.
\end{proof}
...
\begin{theorem}[H\"older's Inequality]
  Let $\lambda_a$, $\lambda_b$, \dots,
  $\lambda_z$ be positive reals with $\lambda_a + \lambda_b + \dots + \lambda_z = 1$.
  Let $a_1$, $a_2$, \dots, $a_n$, $b_1$, $b_2$, \dots, $b_n$, \dots, $z_1$, $z_2$, \dots,
  $z_n$ be positive reals.
  Then
  ...
  Equality holds if $a_1 : a_2 : \dots : a_n \equiv b_1 : b_2 : \dots : b_n \equiv \dots \equiv z_1 : z_2 : \dots
  : z_n$.
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  \[
    ...
    \ge \left( a+b+c \right)
  \]
  So it suffices to prove $(a+b+c)^3 \ge \sum_{\text{cyc}} a(a^2+8bc) = a^3+b^3+c^3+24abc$.
