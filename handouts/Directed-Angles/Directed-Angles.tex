...
Thus any time you want to add two angles,
you technically need to also check that point $P$ lies ``inside'' $\angle AOB$.
...
I've almost never seen them addressed seriously,
except in the very rare circumstances in which they actually matter.
...
\begin{figure}[ht]
  ...
  \begin{asy}
    ...
    MP("\ell", L, L);
    MP("m", M, M);
    draw(arc((0,0),0.4*L,0.4*M), EndArrow);
    MP("50^{\circ}", 0.6*dir(105), (0,0));
  \end{asy}
...
\begin{figure}[ht]
  ...
  \begin{asy}
    ...
    label("$m$", M, M);
    draw(arc((0,0),0.4*L,0.4*M), EndArrow);
    draw(arc((0,0),-0.3*M,0.3*L), dotted, BeginArrow);
    dot("$A$", 0.8*L, dir(-10));
    dot("$B$", -0.6*M, dir(20));
    dot("$O$", (0,0), dir(190));
    label("$50^{\circ}$", 0.6*dir(105), (0,0));
    label("$-130^{\circ}$", 0.5*dir(15), (0,0));
  \end{asy}
...
Now you should be paying attention,
because I just eliminated one of the biggest pains in olympiad geometry for you.
...
\begin{figure}[ht]
  ...
  \begin{asy}
    ...
    real k = 0.6;
    label("$\measuredangle CAB$", k*dir(A), (0,0));
    label("$\measuredangle ABC$", k*dir(B), (0,0));
    label("$\measuredangle BCA$", 0.9*k*dir(C), (0,0));
  \end{asy}
...
\begin{center}
  \begin{asy}
    ...
    pair E = Drawing("E'", 2*foot((0,0),M,Ha)-Ha, dir(70));
    Drawing(A--Ha--B--Hc--C);
...
\begin{proof}
  [Solution]
  Let $H_A$ and $H_C$ be the reflections of $H$ across $\ol{BC}$ and $\ol{BA}$;
  it is well-known that these lie on $\Gamma$.
  Let $E'$ be the second intersection of line $H_AM$ with $\Gamma$.
  By construction, lines $E'M$ and $HK$ concur on $\ol{BC}$, and our goal is to show that $B$, $L$, $E'$,
  $M$ are concyclic.
...
\begin{center}
  ...
  \begin{asy}
    ...
    pair B = Drawing("B", dir(210), dir(210));
    pair C = Drawing("C", dir(330), dir(330));
    pair I = Drawing((pair)incenter(A,B,C));
