...
\begin{itemize}
  ...
  $0 \le (a - b)^2 = (a + b)^2 - 4ab = (10 - c)^2 - 4\left( 25 - c(a + b) \right)
    = (10 - c)^2 - 4\left( 25 - c(10 - c) \right) = c(20 - 3c)$,
  write instead
  \begin{align*}
    0 &\le (a - b)^2\\
     &= (a + b)^2 - 4ab \\
...
are too verbose and want advice on how to trim them down.
...
I think in a lot of cases students think there's a sort of dial on
...
But I have found that ``too much detail'' is usually \emph{not} the issue.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    and $X$ is equivalent to $Y$, but $X$ is given, so $Z$ holds''.\footnote{One
...
\end{example}
The answer to this problem turns out to be $\binom{n-1}{2}+1$, achieved by setting
\begin{equation}
...
Here are again two ways you could write that verification.\footnote{The
  verbose example is adapted from a real write-up, with permission.}
...
all $(i,j)$ are interesting \emph{except} for the case where $j = n$ and $i \in \{1,2,\dots,n-2\}$.
Indeed:
\begin{itemize}
...
\end{itemize}
But if $i \le n-2$ and $j=n$, then for any $1 \le k < \ell \le n$ we have
\[ a_n - a_i \geq 2^{n-2} - 2^{n-4} > 2^{n-3} = \frac{a_n - a_1}{2}
  \ge \frac{a_\ell - a_k}{2} \]
so these $n-2$ pairs are uninteresting.
This gives a count of $\binom{n}{2} - (n-2) = \binom{n-1}{2}+1$ interesting pairs.

\subsubsection*{💬 Commentary.}
Nothing in the verbose solution is unclear or incomplete \emph{per se},
unlike some of the bad solutions we showed in \Cref{sec:examples}.
(The bad examples in \Cref{sec:examples} were missing so much explanation
that a reader might not understand them at all.)
Still, note how much difference a little bit of planning and organization can make.
