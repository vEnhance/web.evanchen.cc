...
\newkeytheorem{CRT}[style=thmbluebox,name={Chinese Remainder Theorem}]
...
\begin{proof}
  ...
  $t^2+t+1$.
  So, we've managed to eliminate almost all the structure of the problem before
  using the Chinese Remainder Theorem, leading us to this heart of the problem.
...
\begin{problem}
  ...
  \[ \frac{a_1}{b_1}, \quad \frac{a_2}{b_2}, \quad \dots,
    \quad \frac{a_k}{b_k} \]
  of rational numbers, where $a_i$, $b_i$ are relatively prime positive integers for each $i=1,2,\dots,k$,
  and moreover
...
\begin{problem}
  ...
  \begin{hint}
    ...
    we have $x-y \notin A_i$.
    Show greedily that we can have \[ \left\lvert B_i \right\rvert \ge
    \frac{m_i}{2\left\lvert A_i \right\rvert+1}. \]
  \end{hint}
\end{problem}
