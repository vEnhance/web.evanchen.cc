...
In particular, we'll use a capital letter (like $Z$) to denote the point
associated to a complex number (like $z$).
...
\begin{example}
  ...
  Let $S$, $T$, $U$ respectively denote the reflections of $D$, $E$, $F$ across $\ol{BC}$, $\ol{CA}$,
  $\ol{AB}$.
  Prove that points $S$, $T$, $U$, $H$ are concyclic.
\end{example}
\begin{proof}
  Let $(ABC)$ be the unit circle and $h = a+b+c$.
  WLOG, $\ol{AD}$, $\ol{BE}$,  $\ol{CF}$ are perpendicular to the real axis (rotate appropriately);
  thus $d = \ol a$ and so on.
...
\begin{soln}
  ...
  \[ q = \frac{dd(g+h)-gh(d+d)}{d^2-gh}
    = \frac{\frac{1}{x^2} \left( -\frac1y-\frac1z \right) - \frac{1}{yz}
    \frac{2}{x}}{\frac{1}{x^2} - \frac{1}{yz}}
    = \frac{2x+y+z}{x^2-yz}. \]
  so
